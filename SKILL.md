...
7. 记录发布源：当前分支、当前提交、最终 tag。示例只读命令：`git branch --show-current`、`git rev-parse HEAD`。
8. push 到远端。默认只推送当前分支和指定 tag；如果本地分支有新增提交，优先使用 `git push --atomic origin <branch> <version>`，否则使用 `git push origin <version>`。不要使用 `git push --tags`。
9. 按“合并到 master”流程把发布 tag 合并到 `master`。
10. 如果用户明确要求打包，按“打包命令选择”流程选择项目脚本；生产镜像优先交给 Jenkins。
11. 输出结果摘要，流程结束。
...
4. 记录源分支和发布 tag 后切换到 `master`：优先使用 `git switch master`，不支持时用 `git checkout master`。
5. 更新 `master`：`git pull --ff-only origin master`。
6. 将发布 tag 合入 `master`：`git merge --ff-only <version>`。如果不能 fast-forward，停止并报告，不自动创建 merge commit。
7. 推送 `master`：`git push origin master`。
8. 如果后续还要继续在源分支工作，合并结束后可切回原分支；不要在未确认时改动源分支。
...
完整 Jenkinsfile 示例维护在仓库 README 中；SKILL.md 只保留执行判断和核心命令。
...
3. 普通静态生产包：用户说“普通打包”“静态包”“pnpm build 打包”时，使用 `pnpm build`，通常输出 `dist/`。
